...
/*!!!!!!!!!!!DO NOT CHANGE END!!!!!!!!!!!*/
  ...
  init() {
    ...
    if (cpu && memory) {
      set(this, 'cpuAndMemoryConfig', `${ get(this, 'config.cpu') }/${ get(this, 'config.memory') }`);
    }
  },
  ...
  selectedCpuAndMemory: computed('cpuAndMemoryConfig', function() {
    const cm = get(this, 'cpuAndMemoryConfig');
    const choices = get(this, 'cpuAndMemoryChoices') || [];
  ...
  cpuAndMemoryChoices: computed('config.instanceType', 'config.zone', 'instanceTypeChoices', function() {
    const instanceType = get(this, 'config.instanceType');
    const zone = get(this, 'config.zone');
    const instanceTypeChoices = get(this, 'instanceTypeChoices') || [];
    ...
    return found.flavors.filter((f) => f.cpuCount >= 4 && f.memoryCapacityInGB >= 8).map((f) => ({
